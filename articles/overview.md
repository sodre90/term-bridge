...
- **Agent inbox** — answer blocking prompts (permission requests, questions) via `POST /feed/{id}/reply`. cmux hosts only for now: a tmux host advertises `feed: false` and the Inbox, YOLO and the pending-count poll are hidden for it until the Claude Code hooks land ([linux-tmux-host design](../docs/superpowers/specs/2026-09-14-linux-tmux-host-design.md)).
